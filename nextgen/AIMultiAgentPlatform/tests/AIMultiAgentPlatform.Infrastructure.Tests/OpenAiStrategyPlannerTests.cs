...
public sealed class OpenAiStrategyPlannerTests
{
    [Fact]
    public async Task GenerateAsync_SendsStructuredRequestAndParsesJsonPayload()
    {
            ...
            var responseJson =
                """
                {
                  "choices": [
                    {
                      "message": {
                        "content": "{\"strategicNarrative\":\"Narrative\",\"contentPillars\":[\"Pillar 1\",\"Pillar 2\"],\"backlogBlueprintItems\":[]}"
                      }
                    }
                  ],
                  "usage": {
        ...
        var planner = new OpenAiStrategyPlanner(
            new OpenAiOptions(
                true,
        ...
        Assert.Contains("\"strategicNarrative\":\"Narrative\"", result.StrategyBlueprintJson, StringComparison.Ordinal);
        ...
        Assert.Contains("\"response_format\"", capturedBody!, StringComparison.Ordinal);
        Assert.Contains("\"json_schema\"", capturedBody!, StringComparison.Ordinal);
        Assert.Contains("\"model\":\"gpt-5-mini\"", capturedBody!, StringComparison.Ordinal);
    }
    ...
    public async Task GenerateAsync_ReturnsFailureWhenResponseIsUnsuccessful()
    {
        ...
        var result = await planner.GenerateAsync(
            new StrategyPlannerRequest("tenant_001", "corr-001", "strategy-blueprint-v1", string.Empty, "system", "user"),
            CancellationToken.None);
